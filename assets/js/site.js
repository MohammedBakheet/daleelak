...
async function renderDetail() {
  ...
  try {
    ...
    const webcal = c.icsUrl.replace(/^https?:/, 'webcal:');
    root.innerHTML = `
      <div class="detail-hero"><div class="calendar-logo large">${escapeHtml((c.organization || 'د').trim().charAt(0))}</div><div><p class="org">${escapeHtml(c.organization)}</p><h1>${escapeHtml(c.title)}</h1><p>${escapeHtml(c.description)}</p></div></div>
      <div class="detail-grid">
        <section class="panel"><h2>معلومات التقويم</h2><dl class="meta-list detail-meta">
        ...
        <section class="panel subscribe-panel"><h2>إضافة التقويم</h2>
          <a class="btn btn-primary wide" href="${webcal}">اشتراك عبر Apple Calendar</a>
          <button class="btn btn-secondary wide" type="button" data-copy="${c.icsUrl}">نسخ رابط الاشتراك</button>
          <a class="btn btn-secondary wide" href="${c.icsUrl}" download>تنزيل ملف ICS</a>
          <p class="hint">في Google Calendar وOutlook اختر إضافة تقويم من رابط، ثم الصق الرابط المنسوخ.</p>
        </section>
      </div><div class="notice">التحديثات عبر الاشتراك تعتمد على جدول المزامنة في تطبيق التقويم المستخدم، وقد لا تظهر فورًا.</div>`;
    const btn = qs('[data-copy]', root);
    btn?.addEventListener('click', async () => {
      try { await navigator.clipboard.writeText(btn.dataset.copy); btn.textContent = 'تم نسخ الرابط'; setTimeout(()=>btn.textContent='نسخ رابط الاشتراك',1800); }
      catch { prompt('انسخ رابط الاشتراك:', btn.dataset.copy); }
    });
  } catch (error) { root.innerHTML = `<div class="notice error">${escapeHtml(error.message)}</div>`; }
}
